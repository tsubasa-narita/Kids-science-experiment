  ...
  return (
    <main>
      ...
      {screen === "home" && <section className="home">
        ...
        <h1>みて、よそうして、<br /><em>やってみよう！</em></h1>
      ...
      {screen === "preview" && <section className="flow-screen preview-screen">
        <p className="eyebrow">ひらく 紙の花</p><h2>おみずに うかべたら<br /><em>どうなるかな？</em></h2>
      ...
      {screen === "guess" && <section className="flow-screen">
        <p className="eyebrow">よそう タイム</p><h2>花びらは<br /><em>どうなると おもう？</em></h2>
      ...
      {screen === "check" && <section className="flow-screen guardian-screen">
        <p className="eyebrow">おとなの かたへ</p><h2>はじめる まえに<br /><em>4つ チェック</em></h2>
      ...
      {screen === "steps" && <section className="flow-screen steps-screen">
        <p className="progress">{step + 1} / {flowerSteps.length}</p><span className={`role role-${flowerSteps[step][0]}`}>{flowerSteps[step][0]} の しごと</span><div className="step-symbol">{flowerSteps[step][3]}</div>
        <h2>{flowerSteps[step][1]}</h2><p className="step-copy">{flowerSteps[step][2]}</p><SpeakButton text={flowerSteps[step][2]} />
      ...
      {screen === "observe" && <section className="flow-screen observe-screen">
        <p className="eyebrow">みつけた！</p><h2>どんなふうに<br /><em>なった？</em></h2>
        <div className={`observation-scene ${observation || "waiting"}`}><b>{observation === "open" ? "✿" : observation === "little" ? "❀" : observation === "still" ? "⊙" : "？"}</b><span>⌁ ⌁ ⌁</span></div>
      ...
      {screen === "why" && <section className="flow-screen why-screen">
        <p className="eyebrow">ふしぎの ひみつ</p><h2>紙は おみずを<br /><em>すいこむんだ。</em></h2>
      ...
      {screen === "coming" && <section className="flow-screen coming"><p className="eyebrow">{selected}</p><div className="coming-icon">✦</div><h2>この ふしぎは<br /><em>じゅんび中！</em></h2><p className="lead">もうすぐ カードから よそうして<br />あそべるように なるよ。</p><button className="primary" onClick={reset}>ほかの カードを みる <span>→</span></button></section>}
